...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& mask_H,
    const torch::Tensor& max_freqs,
	const bool debug)
{
  ...
  torch::Tensor test_freq_values = torch::zeros({P}, float_opts.device(torch::kCUDA)).requires_grad_(false);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		test_freq_values.contiguous().data<float>(),
        mask_H.contiguous().data<float>(),
        max_freqs.contiguous().data<float>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, test_freq_values);
}
